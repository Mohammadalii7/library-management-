...
class TableSSP {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = TableSSP::pluck( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = TableSSP::bind( $bindings, '%'.$str.'%', \PDO::PARAM_STR );
     ...
     *
     */
    static function simple ( $request, $table, $primaryKey, $columns, $totalCountQuery, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '', $debug = false)
    {
        $bindings = array();
        // Build the SQL query string from the request
        $limit = TableSSP::limit( $request, $columns );
        $order = TableSSP::order( $request, $columns, $joinQuery );
        $where = TableSSP::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            $col = TableSSP::pluck($columns, 'db', $joinQuery);
       ...
       $having
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", TableSSP::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
       $having
        ...
        $data = TableSSP::sql_exec($bindings,$query);

        // Data set length after filtering
        $resFilterLength = TableSSP::sql_exec( 
            "SELECT FOUND_ROWS()"
        );
        $recordsFiltered = $resFilterLength[0][0];

        // Total data set length
        $resTotalLength = TableSSP::sql_exec($totalCountQuery);
        // $resTotalLength = TableSSP::sql_exec(
        ...
        return array(
            ...
            "data"            => TableSSP::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ($bindings, $sql=null )
    {
        ...
        try {
            ...
            TableSSP::fatal( "An SQL error occurred: ".$e->getMessage() );
        }
